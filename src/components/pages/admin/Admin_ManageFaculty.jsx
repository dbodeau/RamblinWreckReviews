...
export default function Admin_ManageFaculty() {
  ...
  useEffect(() => {
    getDepartment(1)
      .then(dept => {
        const facultyFlattened = dept.faculty.map(p => ({...p, user: undefined, ...p.user, fullName: p.user.first_name + " " + p.user.last_name, source: undefined, invited_by: p.source.first_name + " " + p.source.last_name}))
        setUsers(facultyFlattened)}
      );
  }, [])
